...
- When reverting or re-implementing, you MUST retrieve the original prompt/PR body AND analyze previous agent execution logs — proceeding with only the latest comment loses crucial context.
- A workspace that is dirty after an informational task triggers automatic commit-and-push — always run `git clean -fd` before finishing if you made no code changes.
