...
This skill outlines the mechanical steps and procedures required to conduct a thorough security audit on a target codebase, pull request, or architectural component.
...
1. **Constraint Analysis**: 
   ...
   - Map the exact attack surface exposed by the PR, codebase, or configuration (e.g., exposed API endpoints, new input vectors).
...
1. Generate feedback for the identified issues, labeling them with a severity prefix (e.g., `[CRITICAL]`, `[HIGH]`, `[MEDIUM]`, `[LOW]`).
2. Include the exact location, the formulated attack scenario, and a concrete, implementable code remediation for each vulnerability discovered.
