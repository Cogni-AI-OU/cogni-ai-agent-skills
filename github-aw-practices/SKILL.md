...
This skill provides organizational practices, rollout strategies, and A/B experiment specifications for maintaining repositories with GitHub Agentic Workflows (gh-aw).
...
Together they form a defense-in-depth model: integrity filtering keeps untrusted content out of the agent's context, and safe-outputs ensure the agent can only produce authorized side-effects.
...
- Shadow evaluation is needed when the question is whether the real write path behaves correctly on a safe non-production target.
