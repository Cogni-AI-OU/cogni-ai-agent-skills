...
- Performing general-purpose code reviews or static analysis unrelated to agent session behavior or telemetry.
- Analyzing infrastructure or web server logs that are not generated by AI agent workflows.
- Designing new agent personas or skills without first auditing existing logs for improvement signals.

## Common Pitfalls

- **Log Truncation**: GitHub Actions truncates log output beyond ~4MB. Always check for truncation markers before concluding an action was not performed.
- **Run ID vs Branch Matching**: Multiple workflow runs may share the same branch name; always match by run ID, not branch, to avoid analyzing the wrong execution.
- **Skill Loading Latency**: The first skill load in a session is measurably slower than subsequent loads. Factor this out when analyzing initialization telemetry or time-to-first-tool-use.
- **Missing Telemetry**: Not all runtimes expose token usage or tool call counts. When telemetry is absent, report what is available and explicitly note the gap.
