...
- **Idempotent Skill Loading**: Avoid redundant `skill` tool calls for the same skill in the same conversation turn or session. Once a skill is loaded, its instructions are active for the duration of the agent's operation in that context.
